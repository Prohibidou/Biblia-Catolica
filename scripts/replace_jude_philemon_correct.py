...
jude_comments[(1, 20)] = jds_17_25_general + "<br><br>" + jds_20_21_comment
jude_comments[(1, 21)] = jds_17_25_general + "<br><br>" + jds_20_21_comment

# Jds 22-23
jds_22_23_comment = "Los cristianos han de tratar siempre con misericordia a los que se apartan de la buena doctrina, a la vez que evitan el peligro para sus almas. «Es propio de los perfectos que en los pecadores no odien más que los pecados; y que amen a esos mismos hombres» (S. Agustín, Contra Adimantum 17,5)."
jude_comments[(1, 22)] = jds_17_25_general + "<br><br>" + jds_22_23_comment
jude_comments[(1, 23)] = jds_17_25_general + "<br><br>" + jds_22_23_comment

# Jds 24-25
jds_24_25_comment = "La doxología o alabanza a Dios Padre por medio de Jesucristo enseña que Jesús es el Mediador tanto de nuestra salvación como de nuestra alabanza al Padre. Desde sus inicios, la Iglesia tiene la costumbre de dirigir la oración litúrgica al Padre por medio de Jesucristo. San Beda, comentando el v. 25, escribe: «Este versículo asigna al Padre y al Hijo igual y coeterna gloria y poder por los siglos de los siglos. Y acusa de estar en el error a quienes creen que el Hijo es inferior y posterior que el Padre, pues dice que la gloria, la majestad, el imperio y la potestad son para el Padre por medio de Jesucristo, nuestro Señor. Y esto no empezó en algún momento, sino desde antes de los siglos, y ahora, por todos los siglos. Amén» (In Epistolam Iudae, ad loc.)."
jude_comments[(1, 24)] = jds_17_25_general + "<br><br>" + jds_24_25_comment
jude_comments[(1, 25)] = jds_17_25_general + "<br><br>" + jds_24_25_comment

print("Replacing Jude and Philemon comments with correct verse-specific assignments...")

# Decompress
gz_path = "public/bibles/navarra.sqlite.gz"
db_path = "scripts/temp_replace_jude_phm.sqlite"
...
# First, clear all existing comments in Jude and Philemon
print("\nClearing existing comments...")
cursor.execute("UPDATE verses SET comment = NULL WHERE book = 'JUD'")
jude_cleared = cursor.rowcount
cursor.execute("UPDATE verses SET comment = NULL WHERE book = 'PHM'")
phm_cleared = cursor.rowcount
print(f"Cleared {jude_cleared} comments from Jude")
print(f"Cleared {phm_cleared} comments from Philemon")

# Now add the correct comments
...
print(f"Updated {jude_updated} verses in Jude")
print(f"Updated {phm_updated} verses in Philemon")
...
print(f"\nVerification:")
print(f"JUD: {jude_count}/25 verses now have comments")
print(f"PHM: {phm_count}/25 verses now have comments")

# Sample
print("\nSample comments:")
cursor.execute("SELECT verse, comment FROM verses WHERE book = 'JUD' AND chapter=1 ORDER BY verse LIMIT 3")
for r in cursor.fetchall():
    print(f"  Jude 1:{r[0]} - {r[1][:100]}...")

cursor.execute("SELECT verse, comment FROM verses WHERE book = 'PHM' AND chapter=1 ORDER BY verse LIMIT 3")
for r in cursor.fetchall():
    print(f"  Philemon 1:{r[0]} - {r[1][:100]}...")
