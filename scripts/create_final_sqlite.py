...
json_file = 'scripts/biblia_navarra_completa.json'
db_file = 'public/bibles/navarra.sqlite'

if os.path.exists(db_file):
    os.remove(db_file)
if os.path.exists(db_file + '.gz'):
    os.remove(db_file + '.gz')
...
# Tamaño final
size_mb = os.path.getsize(db_file + '.gz') / (1024 * 1024)
print(f"✓ Creado {db_file}.gz ({size_mb:.2f} MB)")
